...
namespace SparseMatrixRepSeqNamespace
{
    ...
    public class SLAE : IDisposable
    {
        ...
        /// <summary>
        /// Вывод в консоль проверки результатов расчета СЛАУ
        /// </summary>
        /// <returns></returns>
        public async Task PrintCheckResult()
        {
            ...
            {
                Console.WriteLine($"Ошибка! Размер вектора правой части СЛАУ ({rightSide.Length}) не соответствует размеру вектора результата ({result.Length})");
                return;
            }

            int numRows = await SparseMatrixRepSeq.GetNumRowsAsync();

            if(numRows != result.Length)
            {
                Console.WriteLine($"Ошибка! Число строк (столбцов) матрицы ({numRows}) не соответствует размеру вектора результата ({result.Length})");
                return;
            }
            ...
            {
                Console.Write($"Проверка строки {i}: ");
                double[] matrixRow = await SparseMatrixRepSeq.GetRowAsync(i);
                double calculatedResult = 0;
                for (int j = 0; j < matrixRow.Length; j++)
                {
                    calculatedResult += matrixRow[j] * result[j];
                }

                double deviation = calculatedResult - rightSide[i];
                Console.WriteLine($"{calculatedResult} - {rightSide[i]} = {deviation}");

                if(deviation > maxDeviation)
                {
                    maxDeviation = deviation;
                    maxDeviationIndex = i;
                    maxDeviationPercent = (calculatedResult - rightSide[i]) * 100 / rightSide[i];
                }
            }
            Console.WriteLine($"----------------------------");
            Console.WriteLine($"Максимальное отклонение: {maxDeviation} в строке {maxDeviationIndex} ({maxDeviationPercent} %)");
            Console.WriteLine($"----------------------------");
        }
        #endregion
